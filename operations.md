...
Envelope: required `operation` (one of the twelve below), required nonempty `target` object; optional nullable `payload` object, `dry_run` boolean (default false), optional non-null `include_snapshot` boolean (default true), `expected_sequence` integer >= 0, `note` string <= 255. Never omit the guard in agent writes. Use `{"survey":true}` when the operation consumes no target; `{}` and `[]` fail Laravel's required rule. Invalid envelopes return Laravel 422 `{message,errors}`; invalid operations/snapshots return controller 422 `{success:false,message}`. Missing/ambiguous named references are 422, not HTTP resource 404. A stale guard returns 409; refetch rather than overwrite.

For token-efficient preview and apply, add `"include_snapshot":false` to the envelope. Only `data.survey.snapshot` is omitted from the response; validation, diff, metadata and sequence remain unchanged, and full revisions are still stored. The diff lists changed targets/fields, not old/new values. Read the target question/page first and read it back after apply; use a full preview when you need to inspect the complete candidate. See the compact read endpoints and sequence-consistency rules in [workflow](workflow.md).
